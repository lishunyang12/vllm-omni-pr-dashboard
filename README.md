...
Public page: https://lishunyang12.github.io/vllm-omni-pr-dashboard/
...
- The [Topics subpage](https://lishunyang12.github.io/vllm-omni-pr-dashboard/topics/)
...
Run `python update.py` with authenticated GitHub CLI (`gh`) or
...
The hosting server runs `bash publish.sh` hourly at minute 17 via cron.
...
and schedule `bash /path/to/checkout/publish.sh`. It needs Git,
...
http://localhost:18445/ . The static page requires no frontend build.
